...
### Variable Shadowing with Import Alias

When importing a package with alias (e.g., `rst "github.com/sourcegraph/scip/cmd/scip/rst"`), never use the same name for local variables.

**Wrong:**
```go
import rst "github.com/sourcegraph/scip/cmd/scip/rst"

func buildRST(...) *rst.RST {
    rst := &rst.RST{  // ← rst shadows the import, causing "rst.Symbol is not a type"
        ...
    }
    symbolIndex := make(map[string]*rst.Symbol)  // Error!
}
```
...
func buildRST(...) *rst.RST {
    rstTable := &rst.RST{  // ← Use distinct name
        ...
    }
    symbolIndex := make(map[string]*rst.Symbol)  // OK
}
```

The error manifests as `package.Type is not a type` because Go resolves the local variable first, which doesn't have the expected field.
