...
command -v cabal >/dev/null 2>&1 || {
  echo >&2 "Haskell's 'cabal' command is not installed. Please install it first via https://www.haskell.org/ghcup/"
  exit 1
}
...
./.bin/protoc --go_out=./cmd/scip/rst --go_opt=paths=source_relative -I. rst.proto
./.bin/goimports -w ./cmd/scip/rst/rst.pb.go
